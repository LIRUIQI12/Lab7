...
def create_lists(distr, empirical_distr_func):
...
def calculate_criterion(distr_size, range_list, prob_list, freq_list):
    criterion = 0
    all_freq = 0
    all_prob = 0
    all_np = 0
...
if __name__ == '__main__':
    distr = normal_distribution(DISTRIBUTION_SIZE)

    mu, sigma = get_distribution_parameters(distr)
    empirical_distr_func = DisFunction(mu, sigma)

    range_list, prob_list, freq_list = create_lists(distr, empirical_distr_func)
    k = len(range_list)

    f = open('out.csv', 'w')
    sys.stdout = f

    criterion = calculate_criterion(len(distr), range_list, prob_list, freq_list)
